...
export class AdeService {
    ...
    async getMatieres() {
        ...
        const uniqueActivities = this.parseActivities(json.treeActivities?.folder ?? []);
    ...
    async getCreneaux(date: string): Promise<any[]> {
        ...
        const activities: { activity_id: string, name: string | undefined, type: string | undefined }[] = [];
        ...
        if (activitiesJson.treeActivities?.folder) {
            activitiesJson.treeActivities.folder.forEach((folder: any) => parcourirFolder(folder));
        }
        ...
        const result: { matiere_name: string | undefined, type: string | undefined, date: string }[] = [];

        if (eventsJson.events?.event) {
            for (const event of eventsJson.events.event) {
                const activityId = event.$.activityId;
                const matchedActivity = activities.find(a => a.activity_id === activityId);
    ...
    async refreshADEData(): Promise<void> {
        ...
        const createdCourseMaterials = await this.courseMaterialService.postCourse_Material(courseMaterials);

        // 3. Extraire les types de cours
        const sessionTypes = matieres
            .filter(item => item.type && item.name)
            .map(item => {
                const matched = createdCourseMaterials.find(c => c.name === item.name);

                if (!matched) {
                    throw new Error(`Matière non trouvée pour le type de cours : ${item.name}`);
                }
